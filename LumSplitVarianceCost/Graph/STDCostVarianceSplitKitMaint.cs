...
{
    public class STDCostVarianceSplitKitMaint : PXGraph<STDCostVarianceSplitKitMaint>
    {
        public PXFilter<STDCostVarianceSplitKitFilter> MasterFilter;
        public SelectFrom<LumSTDCostVarSplit>.Where<LumSTDCostVarSplit.finPeriodID.IsGreaterEqual<STDCostVarianceSplitKitFilter.fromFinPeriodID.FromCurrent>.
                                                And<LumSTDCostVarSplit.finPeriodID.IsLessEqual<STDCostVarianceSplitKitFilter.toFinPeriodID.FromCurrent>>>.OrderBy<Asc<LumSTDCostVarSplit.inventoryCD>>.View DetailsView;

        public SelectFrom<LumSTDCostVarSplitKitHistory>.Where<LumSTDCostVarSplitKitHistory.finPeriodID.IsEqual<STDCostVarianceSplitKitFilter.toFinPeriodID.FromCurrent>>.OrderBy<Asc<LumSTDCostVarSplitKitHistory.inventoryCD>>.View HistoryDetailsView;

        public STDCostVarianceSplitKitMaint()
        {
        ...
        public IEnumerable detailsView()
        {
            ...
            var fromPeriodID = ((STDCostVarianceSplitKitFilter)this.Caches[typeof(STDCostVarianceSplitKitFilter)].Current)?.FromFinPeriodID;
            var toPeriodID = ((STDCostVarianceSplitKitFilter)this.Caches[typeof(STDCostVarianceSplitKitFilter)].Current)?.ToFinPeriodID;
            if (fromPeriodID != null && toPeriodID != null)
            {
                var curLumSTDCostVarSplitKitHistory = SelectFrom<LumSTDCostVarSplitKitHistory>.Where<LumSTDCostVarSplitKitHistory.finPeriodID.IsEqual<@P.AsString>>.View.Select(this, toPeriodID);
                if (curLumSTDCostVarSplitKitHistory != null && curLumSTDCostVarSplitKitHistory.Count > 0)
                {
                    foreach (LumSTDCostVarSplitKitHistory data in curLumSTDCostVarSplitKitHistory)
                    {
                ...
                PXSPParameter keepKitRecordOnly = new PXSPInParameter("@KeepKitRecordOnly", PXDbType.Bit, 1);
                pars.Add(p0);
                pars.Add(p1);
                pars.Add(companyID);
        ...
        private void insertLumSTDCostVarSplitKitHistory()
        {
            ...
            var curLumSTDCostVarSplitKitHistory = SelectFrom<LumSTDCostVarSplitKitHistory>.Where<LumSTDCostVarSplitKitHistory.finPeriodID.IsEqual<@P.AsString>>.View.Select(this, conditionPeriod);
            
            if (curLumSTDCostVarSplitKitHistory.TopFirst?.LineNbr == null)
            {
                foreach (LumSTDCostVarSplit line in curLumSTDCostVarSplit.OrderBy((x => ((LumSTDCostVarSplit)x).InventoryCD)))
                {
                    this.ProviderInsert<LumSTDCostVarSplitKitHistory>(
        ...
        {
            this.ProviderDelete<LumSTDCostVarSplitKitHistory>(
                new PXDataFieldRestrict("FinPeriodID", finPeriodID));
        }

        #region Actions

        public PXAction<STDCostVarianceSplitKitFilter> saveAndFrozen;
        [PXButton()]
        ...
        {
            var filterCurrnetCache = this.Caches[typeof(STDCostVarianceSplitKitFilter)].Current as STDCostVarianceSplitKitFilter;
            ...
            insertLumSTDCostVarSplitKitHistory();
            setButtonVisible();
            buttonEnable(false);
        ...
        public PXAction<STDCostVarianceSplitKitFilter> deleteAndRetrieve;
        [PXButton()]
        ...
        public PXAction<STDCostVarianceSplitKitFilter> createSplitJournal;
        [PXButton()]
        ...
        protected virtual IEnumerable CreateSplitJournal(PXAdapter adapter)
        {
            ...
            var curLumSTDCostVarSplitKitHistory = SelectFrom<LumSTDCostVarSplitKitHistory>.Where<LumSTDCostVarSplitKitHistory.finPeriodID.IsEqual<@P.AsString>>.View.Select(this, filterCurrnetCache.ToFinPeriodID);
            if (curLumSTDCostVarSplitKitHistory.Where(x => ((LumSTDCostVarSplitKitHistory)x).SplitCost != 0m).Count() == 0) throw new PXException("There is no data because all SplitCost is 0.");
            ...
            if (curLumSTDCostVarSplitKitHistory.TopFirst?.LineNbr != null)
            {
                ...
                DateTime firstDayInFinPeriod = DateTime.ParseExact(curLumSTDCostVarSplitKitHistory.TopFirst.FinPeriodID.ToString() + "01", "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture);
                ...
                foreach (LumSTDCostVarSplitKitHistory line in curLumSTDCostVarSplitKitHistory.Where(x => ((LumSTDCostVarSplitKitHistory)x).SplitCost != 0m))
                {
        ...
        public PXAction<STDCostVarianceSplitKitFilter> setSTDPendingCost;
        [PXButton()]
        ...
        protected virtual IEnumerable SetSTDPendingCost(PXAdapter adapter)
        {
            ...
            if (filterCurrnetCache.FromFinPeriodID == null || filterCurrnetCache.ToFinPeriodID == null) throw new PXException("Please enter dates.");
            var curLumSTDCostVarSplitKitHistory = SelectFrom<LumSTDCostVarSplitKitHistory>.Where<LumSTDCostVarSplitKitHistory.finPeriodID.IsEqual<@P.AsString>>.View.Select(this, filterCurrnetCache.ToFinPeriodID);
            if (curLumSTDCostVarSplitKitHistory.Count == 0) throw new PXException("There is no data.");
            ...
            DateTime lastDayInFinPeriod = DateTime.ParseExact(curLumSTDCostVarSplitKitHistory.TopFirst.FinPeriodID.ToString() + "01", "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture).AddMonths(1).AddDays(-1);
            if (curLumSTDCostVarSplitKitHistory.Where(x => ((LumSTDCostVarSplitKitHistory)x).SplitCost != 0m).Count() == 0) throw new PXException("There is no data because all SplitCost is 0.");
            foreach (LumSTDCostVarSplitKitHistory line in curLumSTDCostVarSplitKitHistory)
            {
            ...
            DateTime firstDayInNextFinPeriod = DateTime.ParseExact(curLumSTDCostVarSplitKitHistory.TopFirst.FinPeriodID.ToString() + "01", "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture).AddMonths(1);
            foreach (LumSTDCostVarSplitKitHistory line in curLumSTDCostVarSplitKitHistory)
            {
                if (line.Newstdcost != null && line.SplitCost != 0m)
                {
        ...
        #region STDCostVarianceSplitKit Filter
        [Serializable]
        [PXCacheName("STD Cost Variance Split Kit Filter")]
        public class STDCostVarianceSplitKitFilter : IBqlTable
        {
